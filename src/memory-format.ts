...
export function parseMemoryFile(raw: string): ParsedMemory {
  ...
  if (format !== MEMORY_FORMAT_VERSION) {
    throw new ProjectMemoryError(
      'UNSUPPORTED_FORMAT',
      `Unsupported Project Memory format ${String(fields.get('format'))}; expected ${MEMORY_FORMAT_VERSION}.`,
    )
  }
...
export function serializeMemoryFile(revision: number, content: string): string {
