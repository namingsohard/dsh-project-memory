...
export async function resolveMemoryPaths(
  workspaceRoot: string,
  createAgentDirectory: boolean,
): Promise<MemoryPaths | undefined> {
  ...
  try {
    ...
    if (!createAgentDirectory) return undefined
    try {
...
export async function assertSafeMemoryFile(paths: MemoryPaths): Promise<boolean> {
  try {
