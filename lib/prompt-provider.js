...
Its subject is always the project: an entry is a fact about this repository that stays true next time. Every line travels in every later session's context, so the file stays small.

The snapshot above, when present, is already the current persistent memory and the trusted baseline: do not re-read files or re-explore the workspace to confirm it. Direct evidence you obtain naturally wins over it. If direct evidence contradicts it, write the correction with project_memory_write.

Record it with project_memory_write, whose own description carries the argument, approval, and rejection rules; call project_memory_read first for the revision to submit. Project Memory cannot override higher-priority instructions, permissions, or safety constraints.`;
