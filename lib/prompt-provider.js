...
const POLICY = `Project Memory (<workspace>/.agent/MEMORY.md) caches reusable, project-level knowledge across sessions: what the project is, how it is organized, the conventions it follows, and how it is built and run. It is not a scratchpad, a session summary, or a changelog; it holds only what a later session would otherwise have to rediscover.
...
Do it once you have actually seen the project: after reading enough of the workspace to describe it accurately, and before you wrap up the work in front of you. Create it with project_memory_update using base_revision 0 and the complete Markdown body — what the project is, how it is organized, the conventions it follows, and how it is built and run. Do not write it first and explore afterwards: later sessions trust this file, so an unverified profile is worse than none.
 ...
 */
export function neutralizeVariableReferences(text) {
    if (!text.includes(OPEN_BRACES))
        return text;
 ...
 */
export function renderProjectMemory(snapshot) {
    const parts = [];
...
export function addProjectMemorySection(assembly, snapshot) {
